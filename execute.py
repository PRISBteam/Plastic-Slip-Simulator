...
def main():
    ...
    try:
        ...
        if STRUC == 'simple cubic':
            ...
            nodes = dcc.build.create_nodes(structure = STRUC,
                                           origin = first_node,
                                           lattice = LATTICE,
                                           size = SIZE,
                                           dim = DIM)
            ...
            edges = dcc.build.find_neighbours(nodes, LATTICE, structure = STRUC, dim=DIM)
                        
            if degree_yes:
                node_degrees = dcc.matrices.degree_distribution(edges, nodes)
            
            #------- FACES in SC
            
            faces = dcc.build.create_faces(edges, structure = STRUC)
            
            faces_as_edges = dcc.orientations.faces_to_edges(faces, edges, edges_per_face = 4)
            
            #------- VOLUMES in SC
            
            volumes = dcc.build.create_volumes(LATTICE, STRUC, cells_0D = nodes)
            
            volumes_as_faces = dcc.orientations.volumes_to_faces(volumes, faces, faces_per_volume = 6)
                        
            volumes_as_faces, faces_as_edges = dcc.orientations.find_relative_orientations(cells_3D = volumes,
                                                                                           cells_2D = faces,
                                                                                           cells_1D = edges,
                                                                                           cells_0D = nodes,
            ...
            adjacency_matrices = dcc.matrices.combinatorial_form(structure = STRUC,
                                                                 degree = 0,
                                                                 cells_3D = volumes,
                                                                 cells_2D = faces,
                                                                 cells_1D = edges,
            ...
            incidence_matrices = dcc.matrices.combinatorial_form(structure = STRUC,
                                                                 degree = 1,
                                                                 cells_3D = volumes,
                                                                 cells_2D = faces,
                                                                 cells_1D = edges,
            ...
            nodes, (nodes_sc, nodes_bcc, nodes_virtual) = dcc.build.create_nodes(structure = STRUC,
                                                                                 origin = first_node,
                                                                                 lattice = LATTICE,
                                                                                 size = SIZE,
                                                                                 dim = DIM,
            ...
            edges, (edges_sc, edges_bcc, edges_virtual) = dcc.build.find_neighbours(nodes, LATTICE,
                                                                                    structure = STRUC,
                                                                                    dim=DIM,
            ...
            faces, faces_sc = dcc.build.create_faces(edges, structure = STRUC, cells_0D = nodes)
            
            faces_as_edges = dcc.orientations.faces_to_edges(faces, edges)
                        
            #------- VOLUMES in BCC
                
            volumes = dcc.build.create_volumes(LATTICE, STRUC, cells_2D = faces)
            
            volumes_as_faces = dcc.orientations.volumes_to_faces(volumes, faces)
            ...
            nodes, (nodes_sc, nodes_bcc, nodes_fcc) = dcc.build.create_nodes(structure = 'bcc',
                                                                             origin = first_node,
                                                                             lattice = LATTICE,
                                                                             size = SIZE,
                                                                             dim = DIM)
            ...
            edges, edges_sc, edges_bcc_fcc, edges_fcc2, edges_fcc_sc = dcc.build.find_neighbours(nodes,
                                                                                                 lattice = LATTICE,
                                                                                                 structure = STRUC,
                                                                                                 dim = DIM,
            ...
            faces, faces_slip = dcc.build.create_faces(edges,
                                                       structure = STRUC,
